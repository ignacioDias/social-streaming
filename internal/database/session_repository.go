...
func (sr *SessionRepository) CreateSession(ctx context.Context, session *domain.Session) error {
	...
	`
	stmt, err := sr.db.PrepareNamedContext(ctx, query)
	if err != nil {
		return err
	}
...
func (sr *SessionRepository) FindSessionByID(ctx context.Context, id string) (*domain.Session, error) {
	...
	if err := sr.db.GetContext(ctx, &session, query, id); err != nil {
...
func (sr *SessionRepository) DeleteSessionByID(ctx context.Context, id string) error {
...
func (sr *SessionRepository) DeleteSessionsByUserID(ctx context.Context, userID int64) error {
	query := "DELETE FROM sessions WHERE user_id = $1"
	result, err := sr.db.ExecContext(ctx, query, userID)
	ret := checkQueryResult(result, err)
	return ret
}
